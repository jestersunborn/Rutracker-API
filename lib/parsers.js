...
const parseFullInfo = (html) => {
  ...
  const body = $('.post_body').eq(0).ignore('div.sp-wrap').text(); // Parse only first '.post_body', because others are comments, and ignore content in spoller tags.
  ...
  const magnetLink = $(".magnet-link-1").attr("href");

  return { img, body, categories, seed, leech, hash, magnetLink };
};
